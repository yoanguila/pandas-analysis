...
def sheet_summary(df: pd.DataFrame) -> pd.DataFrame:
    summary = df.groupby("product").agg(
        ...
        rev_per_unit=("revenue", lambda x: round(x.sum() / df.loc[x.index, "units"].sum(), 2))
...
def sheet_by_month(df: pd.DataFrame) -> pd.DataFrame:
    ...
    by_month = df.groupby(["month_num", "month_name"]).agg(
        total_revenue=("revenue", "sum"),
        total_units=("units", "sum"),
        num_sales=("revenue", "count")
    ).reset_index().sort_values("month_num").drop(columns="month_num")
    by_month.columns = ["month", "total_revenue", "total_units", "num_sales"]
    return by_month
...
def sheet_top_sales(df: pd.DataFrame) -> pd.DataFrame:
    ...
    top.index += 1
    return top[["date", "product", "category", "region", "revenue", "units", "rev_per_unit"]]
